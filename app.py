...
def overview():
    ...
    nlp_report = session.get(
        "nlp_report",
        "Model forecasting belum menghasilkan summary."
    )
    ...
    if selected_model == "best":
        ...
        rest = [
            model for model in all_keys
            if model not in best_model_names
        ]
    ...
    if request.method == "POST":
        ...
        if "GBR" in active_models:
            add_row(
                "GBR",
                gbr.predict(data_ml)[0]
            )

        if "XGB" in active_models:
            add_row(
                "XGB",
                xgb.predict(data_ml)[0]
            )
        ...
        if DL_READY and data_seq is not None:
            ...
            if "LSTM" in active_models:
                ...
                add_row(
                    "LSTM",
                    scaler_y.inverse_transform(raw)[0][0]
                )
